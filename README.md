...
- **39（托管版 · 直接用我部署跑）**：我这边提供线上入口，你直接创建朋友圈房间/发链接和口令给朋友用（不需要你自己部署）。  
  - 说明：这是演示级托管，Render 免费实例可能冷启动；数据不承诺长期保存；不含 UI/功能定制。
- **69（远程部署/排障支持）**：帮你把这套跑起来（Render + Pages）、配环境变量、把 AdminKey/结果可见性等设置走通（部署到你自己的账号更可控）。
